 ...
 * Macros
 ********************************************************************/
 #define EAT_SMS_MAX_LEN                 170
 #define EAT_SMS_NAME_LEN                43
 #define EAT_SMS_DATA_TIME_LEN           25
 #define EAT_SMS_DATA_LEN                350
 #define EAT_SMS_NUMBER_LEN              43
...
{
    u8 name[EAT_SMS_NAME_LEN+1];
    u8 datetime[EAT_SMS_DATA_TIME_LEN+1];
    u8 data[EAT_SMS_DATA_LEN+1];
    u8 number[EAT_SMS_NUMBER_LEN+1];
    u8 status;
    u16 len;
}EatSmsReadCnf_st;
...
{
    EatSmsStorage_en storage;
    u16 index;
}EatSmsNewMessageInd_st;
...
typedef enum
{
   EAT_SMSAL_MTI_DELIVER          = 0x00,
   EAT_SMSAL_MTI_DELIVER_REPORT   = 0x00,
   EAT_SMSAL_MTI_SUBMIT           = 0x01,
   EAT_SMSAL_MTI_SUBMIT_REPORT    = 0x01,
   EAT_SMSAL_MTI_STATUS_REPORT    = 0x02,
   EAT_SMSAL_MTI_COMMAND          = 0x02,
   
   EAT_SMSAL_MTI_UNSPECIFIED      = 0x03,  /* MMI shall handle this case,
                                        * eg, displaying "does not support", 
                                        * or "cannot display", etc */
   EAT_SMSAL_MTI_ILLEGAL          = 0x04
} EatSmsalMti_en;
...
typedef struct {
    ...
    u8 no_orig_addr;
    u8 orig_addr_size;
    u8 orig_addr[EAT_SMS_NUMBER_LEN]; /* assumed to be
                                                         byte-packed*/
...
typedef struct {
    ...
    u8 no_dest_addr;
    u8 dest_addr_size;
    u8 dest_addr[EAT_SMS_NUMBER_LEN]; /* assumed to be
                                                         byte-packed*/
    ...
    u8 user_data_size;
    u8 user_data[EAT_SMS_MAX_LEN]; /* can have user data header also*/
} EatSmsalSubmitPeer_st;

typedef struct
{
   union 
   {
      EatSmsalDeliverPeer_st  deliver_tpdu;
      EatSmsalSubmitPeer_st   submit_tpdu;
   } data;

   EatSmsalConcat_st        concat_info;
   ...
   /* for decoding DCS */   
   EatSmsalMsgClass_en       msg_class;    
   EatSmsalAlphabet_en        alphabet_type;
   eat_bool                   is_compress;

   EatSmsalPort_st          port;
} EatSmsalTpduDecode_st;

typedef struct
{
   EatSmsAddr_st          sca;
   EatSmsalTpduDecode_st tpdu; 
   u8                pdu_len;   /* length of PDU */
   u8                tpdu_len;  /* length of TPDU */
} EatSmsalPduDecode_st;
